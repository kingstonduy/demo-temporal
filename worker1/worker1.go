...
func main() {
	...
	w1.RegisterWorkflow(workflow.ParallelWorkFlow)
	w1.RegisterWorkflow(workflow.AsyncWorkFlow)

	w1.RegisterActivity(activity.RegisterAccount)
	w1.RegisterActivity(activity.RegisterSms)
	w1.RegisterActivity(activity.NotificationSms)
	w1.RegisterActivity(activity.GetBalanceById)
